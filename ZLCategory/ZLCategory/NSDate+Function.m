...
- (NSDate* )getDateFirstTime {
    return [self dateByAddingTimeInterval:-(self.hour*3600+self.minute*60+self.seconds)];
}

- (NSDate* )getDateLastTime {
    NSDate *date = [self dateByAddingTimeInterval:-(self.hour*3600+self.minute*60+self.seconds)];
    return [date dateByAddingHours:24];
}
